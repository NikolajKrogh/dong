...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'session_state') THEN
    CREATE TYPE session_state AS ENUM ('joinable','in_progress','completed');
  END IF;
END
$$;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'participant_membership_type') THEN
    CREATE TYPE participant_membership_type AS ENUM ('registered','guest');
  END IF;
END
$$;
