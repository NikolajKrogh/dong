...
ALTER TABLE public.matches
  ADD CONSTRAINT chk_matches_scores_nonnegative CHECK (home_score >= 0 AND away_score >= 0),
  ADD CONSTRAINT uq_matches_session_id_id UNIQUE (session_id, id);

CREATE UNIQUE INDEX IF NOT EXISTS ux_matches_session_source_match ON public.matches (session_id, source_provider, source_match_id) WHERE source_match_id IS NOT NULL;
