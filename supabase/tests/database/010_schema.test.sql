...
SELECT ok(
        to_regtype('session_state') IS NOT NULL,
        'session_state enum exists'
    );
SELECT ok(
...
SELECT is(
        (
            ...
            WHERE table_schema = 'public'
                AND table_name = 'game_sessions'
                AND column_name = 'common_match_id'
        ),
        'uuid',
...
SELECT is(
        (
            ...
            WHERE table_schema = 'public'
                AND table_name = 'game_sessions'
                AND column_name = 'last_event_sequence'
        ),
        'bigint',
...
SELECT ok(
        to_regclass('public.assignments') IS NOT NULL,
...
SELECT is(
        (
            ...
            FROM information_schema.columns
            WHERE table_schema = 'public'
                AND table_name = 'gameplay_events'
        ),
