...
- **Fix attempted**: `playwright.config.ts` sets `workers: process.env.CI ? 1 : undefined` — serializes every request against the shared dev server in CI while leaving local-dev parallelism untouched. This directly tests the concurrent-load hypothesis. Verification run in progress at time of writing (background-polled; result to be folded in once known).
