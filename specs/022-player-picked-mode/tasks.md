...
- [X] T041 [P] [US6] Extend `__tests__/components/guestJoin/GuestJoinLobby.platform.test.tsx`: in `player_picked` mode a guest sees the pool and the pick control; in `automatic`/`host_assigned` mode, and in a non-`joinable` room, the panel is absent and the card renders as before — depends on T039
...
- [X] T045 [P] Run `npm run db:reset && npm run db:test` ... **Docker was unavailable, so this was done against a local PostgreSQL 17 instance instead**: a fresh database, Supabase-shaped prerequisites (`auth.uid()`, the three roles with `service_role` BYPASSRLS, `pgcrypto`), all **38 migrations applied in order with zero errors**, then every pgTAP suite run through a shim stubbing `plan/ok/is/isnt/matches/finish`. Result: **34 suites, 449 assertions, 0 failures, 0 plan mismatches** — including `010_schema` (37), `220` (26), `230` (41), `240` (33) and the new `250` (36). Cross-check: #184's CI reported 413 assertions over these same 34 files, and 413 + 36 = 449 exactly, which is good evidence the harness agrees with real CI. Schema verified directly against data-model.md: RLS enabled, 1 select-only policy, 3 indexes + PK, 2 `ON DELETE CASCADE` FKs, `authenticated` holding SELECT only, `anon` holding nothing. **Still run `npm run db:test` in CI** — the shim is not pgTAP.
- [X] T046 [P] `npm test`: **83 suites, 443 tests, all passing** (up from 82/428 at branch point), including the pre-existing `AssignmentSection` suite unchanged. `npm run lint`: **0 errors**, 374 warnings vs a 365 baseline — the +9 are `require()`-in-tests and default-import-naming, both already conventional in the files involved. `tsc --noEmit`: **130 errors, exactly the pre-existing baseline**, with an empty new-error diff (measured unfiltered; an earlier filtered count in this branch under-reported and was corrected)
- [X] T047 [P] `.\mvnw.cmd clean verify` in `command-api/`: **BUILD SUCCESS, 76/76 tests passing across 16 test classes**, jar produced. No Java source change was needed, as predicted (research.md R13)
- [X] T048 `npm run bdd:gen` generates both features cleanly (verified meaningful by negative control: an unbound step makes bddgen report "Missing step definitions: 1"). The Playwright suite was then **actually run** -- Chromium was available and Playwright starts Expo itself -- unlike #184, which recorded this as blocked. **All 4 new scenarios pass on both browser projects (8/8)**, and running them is what caught four defects in the new test code: two invalid UTF-8 bytes from a cp1252 slip, an assertion pinning a separator glyph, and a `toBeDisabled()` check that does not apply to non-native controls. Also fixed a latent cross-scenario leak found on the way: `extraSnapshotParticipants` was module-level and never reset, so any scenario injecting members changed `participantCount` -- and therefore `requiredPoolSize` -- for every later scenario in the same worker. **Two scenarios in `configure-start-game.feature` still fail, and they are pre-existing**: they fail identically at the branch point (28557ad), inherited from #135/#184, which never ran this suite. Not caught by CI either -- `client-ci.yml` runs only lint and `test:ci`, with no e2e job. Both stem from `ShellActionButton` rendering a `div` with `aria-disabled` + `pointer-events: none`; one is a mechanical assertion fix, the other has a premise the shipped `startable` gate invalidated, so rewriting it would mean reinterpreting #135's acceptance criteria. Left alone deliberately and filed as a follow-up rather than silently edited
- [X] T049 `quickstart.md` §1 (database layer) walked end to end via T045's harness — every listed assertion group is covered by `250_player_picked_mode.test.sql` and passing, and the schema/grants/RLS were queried directly and match `data-model.md`. §4 (automated coverage) and §5 (Java) are green per T046–T048. **§2 and §3 (manual multi-tab UI walkthroughs) were NOT performed** — they need a browser session driven by hand; the e2e journeys in T048 cover the same paths automatically, but a human pass before merge is still worth doing
- [ ] T050 Add the rollback note from `plan.md` (§Rollback / Recovery) to the PR description: releases modifying persisted data or multiplayer flows must ship rollback/recovery notes per the constitution's delivery workflow — **pending, done when the PR is opened**
