...
- TypeScript 5.3.3 in an Expo SDK 52 / React Native 0.76.9 workspace + Expo Router 4, React 18.3.1, Zustand 5, AsyncStorage, Tamagui 1.141.5, `@tamagui/babel-plugin` 1.141.5, `react-native-toast-message`, `react-native-safe-area-context`, existing `platform/` adapters, and the current shell theme sources in `styles/palette.ts` and `styles/theme.ts` (121-us13-install-tamagui-and-migrate-the-application-shell)
