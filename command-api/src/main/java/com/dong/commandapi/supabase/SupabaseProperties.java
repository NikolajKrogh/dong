 ...
 *
 * <p>{@code @Validated} + {@code @NotBlank} means a missing or blank config
 * value fails bean validation and the application context refuses to start —
 * the fail-closed startup guarantee (FR-013 / research.md ADR-3 §10).
 *
 * @param jwksUrl Supabase JSON Web Key Set discovery endpoint, used to verify
 *                the signature of Supabase-issued JWTs against its published
 *                asymmetric JWT Signing Keys (no shared secret held by this
 *                service) — e.g. {@code https://<project-ref>.supabase.co/auth/v1/.well-known/jwks.json}
...
public record SupabaseProperties(
        ...
        @NotBlank(message = "supabase.jwks-url must be set (fail-closed)")
        String jwksUrl,
